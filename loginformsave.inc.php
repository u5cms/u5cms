...
$sql = "SELECT name, logins FROM resources WHERE logins NOT LIKE '' AND name='" . gnirts_epacse_laer_lqsym($_GET['n']) . "'";
...
if ($num > 0) {
    ...
    if ($unknown != 'ok' || $_SERVER['PHP_AUTH_USER'] == '' || $_SERVER['PHP_AUTH_PW'] == '') {
        $sql = "SELECT name, logins FROM resources WHERE name='" . gnirts_epacse_laer_lqsym($_GET['n']) . "'";
        $result = mysql_query($sql);
...
/////////////////////////////////////////////////////////////////////////////////////////////
if($automaticallyskipintranetlogin!='yes' || $_GET['n'][0]!='!') {
    if ($unknown != 'ok' || $_SERVER['PHP_AUTH_USER'] == '' || $_SERVER['PHP_AUTH_PW'] == '') {
        include('config.php');
        if ($usesessioninsteadofbasicauth == 'no') {
			...
			require('attempt.php');
			echo '<script>parent.location.href="index.php?c=_login&u="+escape(parent.location.href)</script>';
		}
        exit;
    }
}
