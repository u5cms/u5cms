...
if (!isset($f) || $f=='' || !is_file('fileversions/useruploads/'.$f)) { header("HTTP/1.1 404 Not Found"); die(); }
...
if (empty($_GET['h']) && empty($_GET['w']) && isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])) {
...
if (strpos($f,'.jpg') !== false || strpos($f,'.JPG') !== false || strpos($f,'.jpeg') !== false || strpos($f,'.JPEG') !== false) {
//////////////////////////////////////////////////////////
...
if ($_GET['h']>0) {
    ...
    else header("Content-Disposition:inline;filename=".explode('_', basename($_GET['f']), 2)[0].'.'.$ext);
