...
// Database connection parameters;
   $host= 'localhost'; // If your DB server is not running on localhost, enter its address;
   $username = 'enterusernamehere';
   $password = 'enterpasswordhere';
   $db = 'enterdatabasenamehere';
...
// E-mail transport via SMTP ('no' or 'yes')? Some providers demand 'yes', sometimes even TO or FROM must be registered with the respective domain (concerns $mymail on line 3 of this file);
   $usesmtp='no';
   $smtpservername='type_your_smtp_servername_here';
   //If NO encryption, remove all this: 'ssl' => 'ssl', (adjust port assignment or also remove if standard);
   $mysmtpconfigdata = array('auth' => 'login', 'username' => 'type_your_smtp_username_here','password' => 'type_your_smtp_password_here', 'ssl' => 'ssl', 'port' => 465);
   // Do not change this:    'auth' => 'login',
...
// Automatically downsize large std jpgs during upload in the backend. E. g. $limitjpgsizetobytes=1000000; $resizedlongedgepx=1000; $resamplingquality=100;
...
// If your config.php is incomplete or messed up, get its initial structure and values at http://yuba.ch/cp
