...
$sql_a="SELECT name FROM resources WHERE deleted!=1 AND (content_1 LIKE '%[mf]%' OR content_2 LIKE '%[mf]%' OR content_3 LIKE '%[mf]%' OR content_4 LIKE '%[mf]%' OR content_5 LIKE '%[mf]%') AND name='".mysql_real_escape_string($_GET['n'])."'";
$result_a=mysql_query($sql_a);
$num_a = mysql_num_rows($result_a);
if($num_a<1)die('<script>alert("ERROR: [mf] not set in source form!")</script>');
