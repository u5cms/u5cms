...
if($_GET['id']>0) $isid="AND datacsv LIKE '·". gnirts_epacse_laer_lqsym($_GET['id']).";%'";
else $isid='';

$sql_a="SELECT * FROM formdata WHERE $mfwhereclause AND formname='".gnirts_epacse_laer_lqsym($_GET['n'])."' AND authuser!='' AND authuser='".gnirts_epacse_laer_lqsym(u5flatidnlower($_SERVER['PHP_AUTH_USER']))."' $isid ORDER BY id DESC";
